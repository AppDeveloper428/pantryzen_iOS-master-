...
-(RBMenuItem *)initMenuItemWithTitle:(NSString *)title image:(UIImage *)icon withCompletionHandler:(void (^)(BOOL))completion;
{
...
NSString *const MENU_ITEM_DEFAULT_FONTNAME    = @"HelveticaNeue";
NSInteger const MENU_ITEM_DEFAULT_FONTSIZE    = 18;
...
-(void)setShadowProperties{
    ...
    [_contentController.view.layer setShadowColor:[UIColor clearColor].CGColor];
...
//    [menuCell.imageView.layer setCornerRadius:menuItem.icon.size.width/2];
//    [menuCell.imageView.layer setMasksToBounds:YES];
    menuCell.imageView.backgroundColor=[UIColor clearColor];
    menuCell.backgroundView = menuCell.imageView;
    menuCell.selectionStyle=UITableViewCellSelectionStyleNone;
    menuCell.imageView.transform = CGAffineTransformMakeScale(0.3, 0.3);
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(self.highLighedIndex == indexPath.row){
        ...
        [cell.textLabel setFont:[self.titleFont fontWithSize:self.titleFont.pointSize]];
