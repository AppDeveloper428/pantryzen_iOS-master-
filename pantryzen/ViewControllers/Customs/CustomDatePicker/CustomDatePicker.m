...
//    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, _dayImage.size.width + _monthImage.size.width + _yearImage.size.width - 2*TABLE_RECT_OFFSET + 30, _dayImage.size.height);
    ...
    _yearPicker= [[CustomPickerView alloc] initWithFrame:CGRectMake(_dayImage.size.width + _monthImage.size.width-2*TABLE_RECT_OFFSET + 30, 0, _yearImage.size.width, _yearImage.size.height) background:_yearImage itemVerticalOffset:0.0f andData:years];
    ...
    NSArray *monthNames = [df shortMonthSymbols];
    ...
    {
        NSLog(@"Day %d",day);
        _sel_day = [NSString stringWithFormat:@"%d", day];
    }];
    ...
    _sel_year = [NSString stringWithFormat:@"%ld", year];
    _sel_month = [NSString stringWithFormat:@"%ld", month];
    _sel_day = [NSString stringWithFormat:@"%ld", day];
...
//    [self addSubview:_dayPicker];
    ...
    [_yearPicker retrieveCustomPickerViewControllerDidSpinCallback:^(int year)
    {
        ...
        _sel_year = [NSString stringWithFormat:@"%ld", _year];
    }];
    ...
    [_mounthPicker retrieveCustomPickerViewControllerDidSpinCallback:^(int month)
    {
        ...
        NSLog(@"Month %d",month);
        _sel_month = [NSString stringWithFormat:@"%d", month];
    }];
